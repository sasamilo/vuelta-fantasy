(() => {
  ...
  const fallback=n=>{const p=String(n||'').trim().split(/\s+/).filter(Boolean);const ini=p.length?`${p[0][0]}${p.length>1?p[p.length-1][0]:''}`.toUpperCase():'?';return`data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 400"><rect width="300" height="400" fill="#d9ff43"/><text x="150" y="220" text-anchor="middle" font-family="Arial,sans-serif" font-size="88" font-weight="900" fill="#11231f">${esc(ini)}</text></svg>`)};};

  async function loadLeaderboard(){try{const rows=await get('leaderboard?select=*&order=total_points.desc,participant_name.asc');const e=$('[data-leaderboard]');if(e)e.innerHTML=rows.length?rows.map((x,i)=>`<tr><td class="rank">${i+1}</td><td>${participant(x.participant_name)}</td><td class="number"><strong>${x.total_points}</strong></td><td class="number">${x.stages_scored}</td></tr>`).join(''):'<tr><td colspan="4" class="empty">No scored predictions yet.</td></tr>';}catch(e){console.error('Leaderboard:',e);empty('[data-leaderboard]',4,'Scores are temporarily unavailable.');}}
  ...
  async function loadCurrentStage(){
    ...
    if(winner){const card=$('[data-stage-winner]'),img=$('[data-winner-image]');if(card&&img){const fb=fallback(winner.rider_name);img.onerror=()=>{img.onerror=null;img.src=fb;};img.src=winner.rider_image||fb;img.alt=winner.rider_name;const name=$('[data-winner-name]');if(name)name.textContent=routeStage?.winner?.display_name||winner.rider_name;const team=$('[data-winner-team]');if(team)team.textContent=routeStage?.winner?.team||'';card.href=winner.rider_url||'#';card.target='_blank';card.rel='noopener';card.hidden=false;}}
