(() => {
  ...
  async function main(){
    ...
    const results=await get('public_stage_results?select=stage_id,stage_number,stage_name,finish_position,rider_name,points&stage_id=eq.'+encodeURIComponent(id)+'&order=finish_position.asc');
    ...
    const routeStage=route.stages?.[n];
    ...
    if(scores.length)showWinner({name:scores[0].participant_name,display_name:scores[0].participant_name,image:participantImages[firstName(scores[0].participant_name)]||'',team:`${scores[0].points} points today`});
